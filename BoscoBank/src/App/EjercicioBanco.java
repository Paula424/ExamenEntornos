...
public class EjercicioBanco {
	...
	public static String leerCodigoCuenta(Banco banco) {
		...
		} while (!banco.existeCuenta(codigoCuenta) && // con x o X puede salir
	...
	public static void main(String[] args) {
		...
		do {
			...
			case 1: // creación del curso
				char respuesta = ' ';
				if (miBanco != null) {
					System.out.println("Ya hay datos creados del banco " + miBanco.getNombre()
							+ ". Si continúa se perderán. ¿Desea continuar?");
					do {
			...
			case 2: // alta de nueva cuenta
				...
				if (miBanco.agregarCuenta(codigo, titular, dni)) {
					System.out.println(
							"Cuenta agregada con éxito. Hay " + miBanco.getNumeroCuentas() + " cuentas registradas.");
				} else {
			...
			case 4: // reintegro de cuenta
				...
				if (cantidad != -1) {
					if (cantidad > miBanco.informaSaldo(codigo)) {
						System.out.println(
								"El saldo de la cuenta es inferior a la cantidad que desea retirar. ¿Continúa? (S/N)");
						do {
							System.out.println("Introduzca S o N:");
							respuesta = Utilidades.leerRespuesta();
						} while (respuesta != 's' && respuesta != 'n');
